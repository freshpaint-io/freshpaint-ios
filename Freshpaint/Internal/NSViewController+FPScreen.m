...
#if TARGET_OS_OSX
@implementation NSViewController (FPScreen)
...
+ (NSViewController *)seg_rootViewControllerFromView:(NSView *)view
{
    ...
    if (rootViewController.childViewControllers.count > 0) {
        if ([rootViewController conformsToProtocol:@protocol(FPScreenReporting)] && [rootViewController respondsToSelector:@selector(seg_mainViewController)]) {
            __auto_type screenReporting = (NSViewController<FPScreenReporting>*)rootViewController;
...
- (void)seg_viewDidAppear:(BOOL)animated
{
    ...
    if ([top conformsToProtocol:@protocol(FPScreenReporting)] && [top respondsToSelector:@selector(seg_trackScreen:name:)]) {
        __auto_type screenReporting = (NSViewController<FPScreenReporting>*)top;
        [screenReporting seg_trackScreen:top name:name];
        return;
    }

    [[FPAnalytics sharedAnalytics] screen:name properties:nil options:nil];
