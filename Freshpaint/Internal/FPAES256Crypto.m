...
#import "FPAES256Crypto.h"
#import "FPUtils.h"

// Implementation courtesy of http://robnapier.net/aes-commoncrypto

static NSString *const kRNCryptManagerErrorDomain = @"com.freshpaint.crypto";
...
{
    NSData *iv = [FPAES256Crypto randomDataOfLength:kAlgorithmIVSize];
    NSData *salt = [FPAES256Crypto randomDataOfLength:kPBKDFSaltSize];
...
- (NSData *)encrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        FPLog(@"Unable to encrypt data", error);
        return nil;
    }
    return cipherData;
}
...
- (NSData *)decrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        FPLog(@"Unable to decrypt data", error);
        return nil;
    }
...
+ (NSData *)randomDataOfLength:(size_t)length
{
    ...
    int result = SecRandomCopyBytes(kSecRandomDefault,
                                    ...
                                    data.mutableBytes);
    if (result != kCCSuccess) {
        FPLog(@"Unable to generate random bytes: %d", result);
    }
                         ...
                         salt:(NSData *)salt
{
    ...
    if (result != kCCSuccess) {
        FPLog(@"Unable to create AES key for password: %d", result);
    }
