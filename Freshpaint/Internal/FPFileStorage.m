...
@implementation FPFileStorage

- (instancetype)initWithFolder:(NSURL *)folderURL crypto:(id<FPCrypto>)crypto
{
    if (self = [super init]) {
...
- (void)removeKey:(NSString *)key
{
    ...
    if (![[NSFileManager defaultManager] removeItemAtURL:url error:&error]) {
        FPLog(@"Unable to remove key %@ - error removing file at path %@", key, url);
    }
}
...
- (void)resetAll
{
    ...
    if (![[NSFileManager defaultManager] removeItemAtURL:self.folderURL error:&error]) {
        FPLog(@"ERROR: Unable to reset file storage. Path cannot be removed - %@", self.folderURL.path);
    }
...
- (void)setData:(NSData *)data forKey:(NSString *)key
{
    ...
    if (![url setResourceValue:@YES
                        forKey:NSURLIsExcludedFromBackupKey
                         error:&error]) {
        FPLog(@"Error excluding %@ from backup %@", [url lastPathComponent], error);
    }
}
...
- (NSData *)dataForKey:(NSString *)key
{
    ...
    if (!data) {
        FPLog(@"WARNING: No data file for key %@", key);
        return nil;
    }
    if (self.crypto) {
...
+ (NSURL *)applicationSupportDirectoryURL
{
    ...
    if (executableName == nil) {
        executableName = @"freshpaint-test";
    }
    ...
    // if it exists, but isn't a directory... yikes, but make a freshpaint-specific one.
    if (!isDirectory && exists) {
        newStoragePath = [storagePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@-%@", executableName, @"freshpaint"]];
    }
    ...
    if (data) {
        ...
        if (needsConversion) {
            @try {
                ...
                FPLog(@"Unable to convert data from plist object to json; Exception: %@, data: %@", e, data);
...
- (NSData *_Nullable)dataFromJSON:(id)json
{
    ...
    if (error) {
        FPLog(@"Unable to serialize data from json object; %@, %@", error, json);
    }
    return data;
}
    ...
    if (error) {
        ...
        if (result != nil) {
            *needsConversion = YES;
            ...
            FPLog(@"Unable to parse json from data %@", error);
        }
    }
    return result;
}
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:url.path
        ...
        if (![[NSFileManager defaultManager] createDirectoryAtPath:url.path
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&error]) {
            FPLog(@"error: %@", error.localizedDescription);
        }
    }
}
...
- (NSData *_Nullable)dataFromPlist:(nonnull id)plist
{
    ...
    @try {
        ...
        FPLog(@"Unable to serialize data from plist object; Exception: %@, plist: %@", e, plist);
    } @finally {
        if (error) {
            FPLog(@"Unable to serialize data from plist object; Error: %@, plist: %@", error, plist);
        }
    }
    return data;
}
